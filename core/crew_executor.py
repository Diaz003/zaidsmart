...
def run_crew_task(runner, task_id, project_id, agent_name, model_id, description):
            ...
            try:
                ...
                if agent_name == "manager":
                    ...
                    try:
                        ...
                        # BUG 1 FIX: Incluir tanto al manager como a los workers en la lista de agents
                        ...
                        if "tool" in err_msg or "endpoint" in err_msg or "404" in err_msg or "delegat" in err_msg:
                            ...
                            # BUG 3 FIX: Forzar allow_delegation a False para evitar bucles en modo secuencial
                            for a in all_agents:
                                a.tools = []
    ...
    if not result_text:
        err_msg = f"Fallo Crítico: Los modelos gratuitos no responden o son incompatibles. {last_exc}"
